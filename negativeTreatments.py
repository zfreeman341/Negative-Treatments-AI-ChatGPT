...
def extract_negative_treatments(slug):
  ...
  negative_treatment_indicators = ['overrul', 'disagree', 'limit', 'unconstitutional', 'preempt', 'incorrect', 'overrul']
  ...
  for paragraph in paragraphs_to_process:
    ...
    while retries > 0:
      try:
        ...
        model="gpt-3.5-turbo",
        messages=[
          {"role": "system", "content": "You are a legal assistant helping me identify negative treatment of other cases in this paragraph."},
          {"role": "system", "content": f"{paragraph} Does this paragraph indicate negative treatment of a legal case? Identify which one. Structure your response like so, Negatively treated case: [case name], nature of the negative treatment: [nature], explanation for why you determined this case was negatively treated: [explanation]"}
        ]
        )
